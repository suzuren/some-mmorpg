...
--socket.read(id, sz) 从一个 socket 上读 sz 指定的字节数。如果读到了指定长度的字符串，
--它把这个字符串返回。如果连接断开导致字节数不够，将返回一个 false 加上读到的字符串。
...
function CMD.auth (fd, addr)
	...
	socket.start (fd)	-- su start read data
	socket.limit (fd, 8192)

	local type, name, args, response = read_msg (fd)
	print("-- loginslave.lua CMD.auth read_msg 1:",type,name,args,response)
	...
	if name == "handshake" then
		...
		type, name, args, response = read_msg (fd)	-- block ,wait client send data arrive
		assert (type == "REQUEST" and name == "auth" and args and args.challenge, "invalid auth request")
		print("-- loginslave.lua CMD.auth read_msg 2:",type,name,args,response)

		local text = aes.decrypt (args.challenge, session_key)
		assert (challenge == text, "auth challenge failed")
		print("-- loginslave.lua CMD.auth account:",account.id,account.password)
		...
		print("-- loginslave.lua CMD.auth master:",id,master)
	...
	print("-- loginslave.lua CMD.auth challenge:",name,args,args.session)
...
function CMD.challenge (session, secret)
	...
	print("-- loginslave.lua CMD.challenge:",session,t)
