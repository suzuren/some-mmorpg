...
local function send_request (name, args)
	--print ("send_request name[",name,"],session_id[",session_id,"],args[",args,"]")
	print (string.format ("-- send_request name[%s],session_id[%d],args[%s]", name,session_id,args))
...
function RESPONSE:handshake (args)
	print ("-- RESPONSE.handshake func",args)
...
--------------------------------------------------RESPONSE
--+handshake [function: 0x16b0f10]
--+challenge [function: 0x16b1130]
--+auth [function: 0x16b1090]
--------------------------------------------------END
...
local function handle_response (id, args)
	...
	--print("-- handle_response: id[",id,"],s[",s,"],sname[",s.name,"],f[",f,"]")
	print(string.format("-- handle_response: id[%d], args[%s], s[%s], s.nanem[%s], s.args[%s], f[%s]",id,args,s,s.name,s.args,f))
	--print("--------------------------------------------------args")
	--print_r (args)
	--print("--------------------------------------------------s")
	--print_r (s)
...
local function handle_message (t, ...)
	--print(string.format("handle_message: t[%s],",t,...))
	print("-- handle_message",t,...)
	--print("-- handle_message: t[",t,"],...[",...,"]")
	if t == "REQUEST" then
		handle_request (...)
	else
		handle_response (...)
...
--print_r(user)
--print("----------------------------------")
print (string.format ("-- login server connected: fd[%d],name[%s],password[%s]", fd,user.name,user.password))
